  ...
  (testing "when result is a map it is returned and transformed"
    (let [db-entry {:db/id 123323
      ...
      (is (= {:stone-type :mafic-rock}
             (d-h.entity/find-by-id db-context {} :stone-type :mafic-rock)))))
...
(deftest find-all-test
  (testing "when result is nil, nil is returned"
    (let [db-context (test-database-context nil)]
      (is (= nil
             (d-h.entity/find-all db-context {} :stone-type)))))
...
(deftest update-test
  (testing "when found entity is completely different than to be saved, nothing is returned"
    (let [found-entity {:age 39}
      ...
      (is (= []
             (d-h.entity/update! db-context {} :name "Rosa" found-entity to-be-saved)))))
  (testing "when no found entity is provided, nothing is returned"
    (let [found-entity {}
      ...
      (is (= []
             (d-h.entity/update! db-context {} :name "Rosa" found-entity to-be-saved)))
      (is (= []
             (d-h.entity/update! db-context nil :name "Rosa" found-entity to-be-saved)))))
  (testing "when no to-be-saved is provided, nothing is returned"
    (let [found-entity {:age 39}
      ...
      (is (= []
             (d-h.entity/update! db-context {} :name "Rosa" found-entity to-be-saved)))
      (is (= []
             (d-h.entity/update! db-context {} :name "Rosa" found-entity nil)))))
  (testing "when maps overlap, the intersection is returned as db/cas col"
    (let [db-context (test-database-context nil)]
      (is (= [[:db/cas [:name "Rosa"] :age 35 49]]
             (d-h.entity/update! db-context {} :name "Rosa" {:age 35, :name "Rosa"} {:age 49})))
      (is (= [[:db/cas [:name "Rosa"] :age 35 49]]
             (d-h.entity/update! db-context {} :name "Rosa" {:age 35, :name "Rosa"} {:age 49, :country :DE})))
      (is (= [[:db/cas [:name "Rosa"] :age 35 49]]
             (d-h.entity/update! db-context {} :name "Rosa" {:age 35, :name "Rosa", :gender :woman} {:age 49, :country :DE})))
      (is (= [[:db/cas [:name "Rosa"] :age 35 49]
              [:db/cas [:name "Rosa"] :city "Zurich" "Berlin"]]
             (d-h.entity/update! db-context {} :name "Rosa" {:age 35, :name "Rosa", :city "Zurich"} {:age 49, :city "Berlin"}))))))
...
(deftest upsert-foreign-test
  (testing "when entity is found, db/add is returned"
    (let [db-context (test-database-context {:age 45, :name "Prestes"})]
      (is (= [[:db/add [:person/id 4] :person/group [:group/id 17]]]
             (d-h.entity/upsert-foreign! db-context {} :group/id 17 :person/group :person/id 4)))))
  (testing "when entity is not found, result is nil"
    (let [db-context (test-database-context nil)]
      (is (nil?
           (d-h.entity/upsert-foreign! db-context {} :group/id 17 :person/group :person/id 4))))))
  ...
  (testing "when deleting retract entity with look up ref is returned"
    (let [db-context (test-database-context nil)]
      (is (= [[:db/retractEntity [:group/id 17]]]
             (d-h.entity/delete! db-context {} :group/id 17))))))
